...
### NOTES ###
# Modified version of build-pglite.sh with limited parallelism (-j2) for memory-constrained environments
# $INSTALL_PREFIX is expected to point to the installation folder of various libraries built to wasm (see pglite-builder)
#############

# Limit parallelism to prevent OOM in Podman/constrained environments
JOBS=${JOBS:-2}
echo "pglite: Using -j${JOBS} for parallel builds"
...
# Step 2: make and install all except pglite (LIMITED PARALLELISM)
emmake make PORTNAME=emscripten -j${JOBS} || { echo 'error: emmake make PORTNAME=emscripten -j' ; exit 21; }
...
# Step 3.1.2 all the rest of contrib (LIMITED PARALLELISM)
emmake make PORTNAME=emscripten -C contrib/ -j${JOBS} || { echo 'error: emmake make PORTNAME=emscripten -C contrib/ -j' ; exit 31; }
...
# Step 4: make and dist other extensions (LIMITED PARALLELISM)
SAVE_PATH=$PATH
PATH=$PATH:$INSTALL_FOLDER/bin
emmake make OPTFLAGS="" PORTNAME=emscripten -j${JOBS} -C pglite || { echo 'error: emmake make OPTFLAGS="" PORTNAME=emscripten -j -C pglite' ; exit 41; }
...
# Step 5: make and install pglite (LIMITED PARALLELISM)
...
PGLITE_CFLAGS="$PGLITE_CFLAGS $PGLITE_EMSCRIPTEN_FLAGS" emmake make PORTNAME=emscripten -j${JOBS} -C src/backend/ install-pglite || { echo 'emmake make OPTFLAGS="" PORTNAME=emscripten -j -C pglite' ; exit 51; }
